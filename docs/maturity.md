...
| Lifecycle/bootstrap | beta | `ServiceGroup` runs HTTP/RPC/consumer/job/background services behind one shutdown signal, exposes starting/running/draining/stopped/failed phases, and enforces stop-hook/task shutdown timeouts; generated services still need a default HTTP/RPC/MQ/Job orchestration template. |
