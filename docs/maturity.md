...
| Lifecycle/bootstrap | scaffold | `ServiceGroup` primitives exist for HTTP/RPC/consumer/job/background services behind one shutdown signal, including starting/running/draining/stopped/failed phases and stop-hook/task shutdown timeouts; generated services still need a default HTTP/RPC/MQ/Job orchestration template and end-to-end production examples before beta or stable claims. |
