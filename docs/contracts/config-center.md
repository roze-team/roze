...
Config Center is usable for internal pilots and controlled deployments. It must not be marketed as fully stable until management APIs, audit persistence, rollback controls, watch isolation, and permission checks are covered by tests and release-gate evidence.
...
## Management API Required Before Stable

The following capabilities are required before Config Center can be marked `stable`:
...
| audit history | persist who/what/when/source/version/hash |
...
- Management API and permission model are either implemented and tested, or the module remains `beta`.
