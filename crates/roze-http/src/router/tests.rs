...
use crate::rest;
use crate::{
    route_params::RouteParams, ConnectInfo, MatchedPath, NestedPath, OriginalUri, Path, State,
};
...
#[test]
fn router_service_types_preserve_tower_bounds() {
    fn assert_service<T>()
    where
        T: Service<IncomingRequest, Response = HttpResponse, Error = Infallible>
            + Clone
            + Send
            + Sync,
        T::Future: Send,
    ...
    fn assert_make_service<T>()
    where
        T: Service<SocketAddr> + Clone + Send + Sync,
        T::Future: Send,
    {
    }

    assert_service::<Router>();
    assert_service::<MethodRouter>();
    assert_service::<RouterIntoService>();
    assert_make_service::<IntoMakeService>();
    assert_make_service::<IntoMakeServiceWithConnectInfo<SocketAddr>>();
    assert_make_service::<MethodRouterIntoMakeServiceWithConnectInfo<SocketAddr>>();
}
...
fn method_router_merge_panics_on_overlapping_method_not_allowed_fallback() {
    ...
    let right = post(|| async { "right" }).method_not_allowed_fallback(|| async { "right 405" });
...
#[test]
#[should_panic(expected = "Router::method_not_allowed_fallback_service cannot be used with Router")]
...
async fn layers_method_router() {
    let mut router = Router::new().route("/healthz", get(|| async { "ok" }).layer(TestHeaderLayer));
    let response = router
        .call(
            Request::builder()
                .uri("/healthz")
...
#[tokio::test]
async fn router_layer_applies_to_default_method_not_allowed_fallback() {
    let mut router = Router::new()
        .route("/healthz", get(|| async { "ok" }))
        .layer(TestHeaderLayer);
    let response = router
