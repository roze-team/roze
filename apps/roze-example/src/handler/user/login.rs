...
pub(crate) struct LoginLoginReqJson {
    username: String,
    password: String,
}
...
pub(crate) async fn login(
    ...
    Json(body): Json<LoginLoginReqJson>,
    ...
    tracing::info!(event = roze_log::events::APPLICATION_LOGIC_STARTED, protocol = "rest", service = %ctx.config.name, operation = "login", response_kind = "json", request_id = %logic_request_id, trace_id = %logic_trace_id, "REST application logic started");
    ...
    let timeout = timeout_enabled
        ...
        .flatten();
    let logic = crate::logic::login(ctx.clone(), request_ctx, req);
    ...
    match &result {
        Ok(_) => {
            tracing::info!(event = roze_log::events::APPLICATION_LOGIC_COMPLETED, protocol = "rest", service = %ctx.config.name, operation = "login", response_kind = "json", elapsed_ms = logic_started.elapsed().as_millis(), request_id = %logic_request_id, trace_id = %logic_trace_id, "REST application logic completed")
        }
        Err(error) => {
            tracing::error!(event = roze_log::events::APPLICATION_LOGIC_FAILED, protocol = "rest", service = %ctx.config.name, operation = "login", response_kind = "json", elapsed_ms = logic_started.elapsed().as_millis(), code = error.code(), error_kind = error.kind(), request_id = %logic_request_id, trace_id = %logic_trace_id, "REST application logic failed")
        }
    }
    match result {
        Ok(resp) => {
        ...
        Err(mut err) => {
            err = roze_middleware::apply_fallback(
                ctx.config.name.as_str(),
                err,
                roze_middleware::route_fallback(Some(&ctx.config.governance), "login"),
            );
            roze_middleware::finish_route(route_guard, false, err.code().to_string());
            Err(err)
        }
