...
fn render_query_types(out: &mut String, model: &ModelSpec, pascal: &str, item_ty: &str) {
    ...
    for field in model_query_fields(model) {
        if let Some(inner_ty) = optional_inner_type(&field.ty) {
            writeln!(out, "    pub {}: Option<{}>,", field.name, inner_ty).unwrap();
            writeln!(out, "    pub {}_is_null: bool,", field.name).unwrap();
        } else {
...
fn render_sea_orm_query_methods(
    ...
    for field in model_query_fields(model) {
        ...
        if optional_inner_type(&field.ty).is_some() {
            ...
            writeln!(
                out,
                "        if req.{}_is_null {{ select = select.filter(Column::{column}.is_null()); }}",
                field.name
            )
            .unwrap();
        } else {
...
fn render_toasty_query_methods(
    ...
    for field in model_query_fields(model) {
        if optional_inner_type(&field.ty).is_some() {
            ...
            writeln!(
                out,
                "        if req.{}_is_null {{ query = query.and({pascal}::fields().{}().is_none()); }}",
                field.name, field.name
            )
            .unwrap();
        } else {
...
fn render_toasty_index_methods(out: &mut String, model: &ModelSpec, pascal: &str) {
    ...
    for index in model.indexes.iter().filter(|index| index.fields.len() > 1) {
        ...
        let assignments = fields
            .iter()
            .map(|field| format!("{}: Some({}.clone()),", field.name, field.name))
            .collect::<Vec<_>>()
            .join(" ");
        writeln!(out).unwrap();
        if index.unique {
...
fn render_toasty_soft_delete_methods(
    ...
    writeln!(
        out,
        "    pub async fn soft_delete_by_{primary}(db: &mut dyn toasty::Executor, {primary}: &{primary_ty}) -> toasty::Result<{pascal}> {{"
    )
    .unwrap();
    writeln!(
        out,
