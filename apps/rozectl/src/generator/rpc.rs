...
async fn main() -> anyhow::Result<()> {{
    ...
    .await?;
    tracing::info!(event = roze_log::events::SERVICE_REGISTRY_REGISTERED, service = %config.name, protocol = "rpc", addr = %rpc.advertise_addr.unwrap_or(rpc.addr), "service registered");
    ...
    tracing::info!(event = roze_log::events::SERVICE_CONTEXT_INITIALIZED, service = %service_name, protocol = "rpc", "service context initialized");
    ...
    group.add_fn(service_name.clone(), move |shutdown| {{
        let ctx = ctx.clone();
            ...
            async move {{
                tracing::info!(event = roze_log::events::RPC_SERVER_LISTENING, protocol = "rpc", listen_addr = %rpc_addr, "RPC server listening");
                ...
                RpcServer::new(rpc_addr)
                    ...
                    .serve_with_shutdown(rpc_addr, routes, async move {{
                        shutdown.wait().await;
                        tracing::info!(event = roze_log::events::RPC_SERVER_SHUTDOWN_REQUESTED, protocol = "rpc", "RPC shutdown requested");
                    }})
                    .await
                    .map_err(|error| anyhow::anyhow!("RPC service failed: {{error}}"))
            }}
    }});
    tracing::info!(event = roze_log::events::SERVICE_STARTING, service = %service_name, protocol = "rpc", listen_addr = %rpc_addr, "service starting");
    ...
    tracing::info!(event = roze_log::events::SERVICE_REGISTRY_UNREGISTERED, service = %service_name, protocol = "rpc", "service unregistered");
    match &result {{
        Ok(()) => tracing::info!(event = roze_log::events::SERVICE_STOPPED, service = %service_name, protocol = "rpc", "service stopped"),
        Err(_) => tracing::error!(event = roze_log::events::SERVICE_FAILED, service = %service_name, protocol = "rpc", error_kind = "lifecycle", "service failed"),
    }}
    result?;
...
fn render_route_method(spec: &ApiSpec, route: &RestRoute) -> String {
    ...
    out.push_str(&format!(
        "        tracing::info!(event = roze_log::events::APPLICATION_LOGIC_STARTED, protocol = \"rpc\", service = %self.ctx.config.name, operation = {handler:?}, request_id = %request_ctx.request_id(), trace_id = %request_ctx.trace_id(), \"RPC application logic started\");\n        let logic_started = std::time::Instant::now();\n        let result = crate::logic::{handler}({args}).await;\n        match &result {{\n            Ok(_) => tracing::info!(event = roze_log::events::APPLICATION_LOGIC_COMPLETED, protocol = \"rpc\", service = %self.ctx.config.name, operation = {handler:?}, elapsed_ms = logic_started.elapsed().as_millis(), request_id = %request_ctx.request_id(), trace_id = %request_ctx.trace_id(), \"RPC application logic completed\"),\n            Err(error) => tracing::error!(event = roze_log::events::APPLICATION_LOGIC_FAILED, protocol = \"rpc\", service = %self.ctx.config.name, operation = {handler:?}, elapsed_ms = logic_started.elapsed().as_millis(), code = error.code(), error_kind = error.kind(), request_id = %request_ctx.request_id(), trace_id = %request_ctx.trace_id(), \"RPC application logic failed\"),\n        }}\n",
        handler = handler,
...
fn render_rpc_method(spec: &ApiSpec, method: &RpcMethod) -> String {
    ...
    out.push_str(&format!(
        "        tracing::info!(event = roze_log::events::APPLICATION_LOGIC_STARTED, protocol = \"rpc\", service = %self.ctx.config.name, operation = {method:?}, request_id = %request_ctx.request_id(), trace_id = %request_ctx.trace_id(), \"RPC application logic started\");\n        let logic_started = std::time::Instant::now();\n        let result = crate::logic::{method_name}(self.ctx.clone(), request_ctx.clone(), req).await;\n        match &result {{\n            Ok(_) => tracing::info!(event = roze_log::events::APPLICATION_LOGIC_COMPLETED, protocol = \"rpc\", service = %self.ctx.config.name, operation = {method:?}, elapsed_ms = logic_started.elapsed().as_millis(), request_id = %request_ctx.request_id(), trace_id = %request_ctx.trace_id(), \"RPC application logic completed\"),\n            Err(error) => tracing::error!(event = roze_log::events::APPLICATION_LOGIC_FAILED, protocol = \"rpc\", service = %self.ctx.config.name, operation = {method:?}, elapsed_ms = logic_started.elapsed().as_millis(), code = error.code(), error_kind = error.kind(), request_id = %request_ctx.request_id(), trace_id = %request_ctx.trace_id(), \"RPC application logic failed\"),\n        }}\n",
        method_name = method_name,
        method = method.name
    ));
    out.push_str("        match result {\n");
...
mod tests {
    ...
    fn rpc_main_uses_service_group_lifecycle() {
        ...
        assert!(rendered.contains("event = roze_log::events::RPC_SERVER_LISTENING"));
    ...
    fn rpc_server_validates_request_before_logic() {
        ...
        assert!(rendered.contains("event = roze_log::events::APPLICATION_LOGIC_STARTED"));
        assert!(rendered.contains("event = roze_log::events::APPLICATION_LOGIC_COMPLETED"));
        assert!(rendered.contains("event = roze_log::events::APPLICATION_LOGIC_FAILED"));
        assert!(rendered.contains("elapsed_ms = logic_started.elapsed().as_millis()"));
