...
pub fn generate_project(spec: &ApiSpec, out: &Path, force: bool) -> anyhow::Result<()> {
    ...
    fs::write(out.join("README.md"), readme(spec))?;
...
pub fn create_api_project(service: &str, out: &Path, force: bool) -> anyhow::Result<()> {
    let source = api_template(service);
    let api_path = out.join(format!("{}.api", service));
    fs::create_dir_all(out)?;
    let spec = crate::parser::parse_api(&source)?;
    generate_project(&spec, out, force)?;
    fs::write(&api_path, &source).with_context(|| format!("failed to write {}", api_path.display()))?;
    register_workspace_member(out)?;
    Ok(())
}

pub fn create_rpc_project(service: &str, out: &Path, force: bool) -> anyhow::Result<()> {
