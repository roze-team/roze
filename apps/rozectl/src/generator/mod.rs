...
fn render_http_smoke_tests(spec: &ApiSpec, base_url: &str) -> String {
        ...
        .filter(|(_, route)| !route.websocket)
    {
        ...
        writeln!(
            &mut out,
            "    assert_eq!(body.as_ref().and_then(|body| body.get(\"code\")).and_then(|code| code.as_i64()), Some(0), \"generated route must use numeric success code 0\");"
        )
        .unwrap();
        writeln!(
            &mut out,
...
fn format_new_project_rust(out: &Path, mode: GenerateMode) -> anyhow::Result<()> {
    ...
    let build_rs = out.join("build.rs");
    if build_rs.is_file() {
        rust_files.push(build_rs);
    }
    if mode == GenerateMode::Update {
        rust_files.retain(|path| is_framework_owned_rust_file(out, path));
    }
    format_generated_rust_files(out, &rust_files)
}

pub(super) fn format_generated_rust_files(
    out: &Path,
    rust_files: &[PathBuf],
) -> anyhow::Result<()> {
...
fn is_framework_owned_rust_file(out: &Path, path: &Path) -> bool {
...
"#,
    ...
    fn update_formats_framework_owned_rust_without_touching_business_files() {
        ...
        let stale_model_source =
            "// @generated by rozectl. DO NOT EDIT.\npub fn stale_model( )->u64{3}\n";
        fs::write(&model, stale_model_source).expect("stale model source");

        format_new_project_rust(&out, GenerateMode::Update).expect("format update output");
        ...
        assert_eq!(
            fs::read_to_string(&model).expect("deferred stale model source"),
            stale_model_source
        );
        let status = Command::new("rustfmt")
    ...
    fn openapi_document_contains_parameters_bodies_and_schemas() {
        ...
        assert_eq!(
            document["paths"]["/api/v1/users/{id}"]["get"]["responses"]["200"]["content"]
                ["application/json"]["schema"]["properties"]["code"]["enum"][0],
            0
        );
    }
...
"#,
    ...
    fn renders_mock_server_from_api_contract() {
        ...
        assert!(main.contains(r#""code": 0"#));
        assert!(main.contains(r#""msg": "OK""#));
        assert!(main.contains(r#""data": {"#));
